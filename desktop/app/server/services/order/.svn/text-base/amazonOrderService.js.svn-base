...
function getOrders(filters, cb, err) {
    commonDAO.getChannelConfig(channel, service, function (credentials, extra, lastSync) {
        if (credentials.AWSAccessKeyID && credentials.SecretKey && credentials.SellerID && credentials.MarketplaceID) {
            ...
            try {
                ...
                amazonMWS.amazonMwsRequest(listOrders, cb);
            } catch (e) {
                err(e);
            }
        } else {
    ...
    if (result.ListOrdersResponse) {
        ...
        if (responseOrderData.Order) {
            ...
            for (var i in responseOrderData.Order) {
                order = responseOrderData.Order[i];
                var grandTotal = 0;
                var maxDeliveryDate = '';
                if (responseOrderData.Order[i].OrderTotal) {
                    grandTotal = responseOrderData.Order[i].OrderTotal[0].Amount[0];
                }
                if (responseOrderData.Order[i].LatestDeliveryDate) {
                    maxDeliveryDate = responseOrderData.Order[i].LatestDeliveryDate;
                }
                order = {
                    orders_external_id: responseOrderData.Order[i].AmazonOrderId,
                    orders_channel_id: [channel],
                    orders_order_date: responseOrderData.Order[i].PurchaseDate,
                    orders_external_status_id: responseOrderData.Order[i].OrderStatus,
                    orders_status_id: util.amazonOrderStatusEnum[responseOrderData.Order[i].OrderStatus],
                    orders_grand_total: grandTotal,
                    orders_last_updated_timestamp: responseOrderData.Order[i].LastUpdateDate,
                    order_item_estimated_ship_date: responseOrderData.Order[i].LatestShipDate,
