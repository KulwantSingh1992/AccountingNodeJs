		...
		csvStream = csv.parse()
			...
			else //console.log(data);
			{
			  if(validated)	{
				...
				if(data[4] != "") {
					productInfoMap[data[4]]=  data[6];
				}
				if(orderInfo.get(data[3]) != null) {
					var tempMap = orderInfo[data[2]];
					orderMap["productIdInfo"] = productInfoMap;
					orderMap["settlementRefNo"] = data[1];
					orderMap["settlementDate"] = data[0];
					orderMap["commissionFee"] = Number(data[16]) + Number(data[18]) + Number(tempMap["commissionFee"]);
					orderMap["invoiceAmount"] = Number(data[12]) + Number(tempMap["invoiceAmount"]);
					orderMap["salesTax"] = Number(data[13]) + Number(tempMap["salesTax"]);
					orderMap["externalOrderId"] = data[3];
					orderMap["shippingCharges"] = Number(data[13]) + Number(tempMap["shippingCharges"]);
					orderMap["promotionalRebate"] = Number(data[14]) + Number(tempMap["promotionalRebate"]);
					orderMap["fbaFee"] = Number(data[17]) + Number(tempMap["fbaFee"]);
					orderMap["orderStatus"] = data[2];
					orderMap["otherTransactionFee"] = Number(data[18]) + NUmber(tempMap["otherTransactionFee"]);
					orderMap["invoiceId"] = "INV11";
					var totalMarketplaceFee = Number(data[16]) + Number(data[18]) + Number(data[13]);
					orderMap["totalMarketPlaceFee"] = totalMarketplaceFee + Number(tempMap["totalMarketPlaceFee"]);
					orderMap["refundedPayment"] = refundedPayment;
					orderInfo[data[3]] = orderMap;
				} else {
					var orderMap = {};
					if(UtilValidate.isNotEmpty(sku)){
						productIdInfo.put(sku,quantity);
					}
					orderMap["productIdInfo"] = productInfoMap;
					orderMap["settlementRefNo"] = data[1];
					orderMap["settlementDate"] = data[0];
					orderMap["commissionFee"] = Number(data[16]) + Number(data[18]);
					orderMap["invoiceAmount"] = data[12];
					orderMap["salesTax"] = data[13];
					orderMap["externalOrderId"] = data[3];
					orderMap["shippingCharges"] = data[13];
					orderMap["promotionalRebate"] = data[14];
					orderMap["fbaFee"] = data[17];
					orderMap["orderStatus"] = data[2];
					orderMap["otherTransactionFee"] = data[18];
					orderMap["invoiceId"] = "INV11";
					var totalMarketplaceFee = Number(data[16]) + Number(data[18]) + Number(data[13]);
					orderMap["totalMarketPlaceFee"] = totalMarketplaceFee;
					orderMap["refundedPayment"] = refundedPayment;
					orderInfo[data[3]] = orderMap;
				}
				createPaymentTransactions(orderInfo);
			 }
